...
def complete_(data): # [1pts]
    ...
    """
    complete_rows = np.all(np.isfinite(data[:, :-1]), axis=1)
...
def incomplete_(data): # [1pts]
    ...
    """  
    incomplete_rows = np.sum(np.isnan(data[:, :-1]), axis=1) == 1
    return data[incomplete_rows]  
    raise NotImplementedError
...
def unlabeled_(data): # [1pts]
    ...
    """
    unlabeled_rows = np.isnan(data[:, -1])
    return data[unlabeled_rows]
    raise NotImplementedError
